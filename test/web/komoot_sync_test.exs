defmodule Web.Rides.KomootSyncTest do
  ...
  test "enriched tour fields are stored and API stats win over computed ones" do
    stub_komoot(tours: %{"tour_recorded" => [@enriched_tour], "tour_planned" => []})

    assert {:ok, %{imported: 1, failed: 0}} = KomootSync.sync()
    assert [ride] = Rides.list_recorded_rides()
  ...
  test "private tours import hidden from public queries" do
    private = Map.merge(@enriched_tour, %{"id" => 555, "status" => "private"})
    stub_komoot(tours: %{"tour_recorded" => [private], "tour_planned" => []})

    assert {:ok, %{imported: 1}} = KomootSync.sync()
    assert Rides.list_recorded_rides() == []
    assert [ride] = Rides.list_recorded_rides(public: false)
    assert ride.visibility == "private"
    assert Rides.get_public_ride(ride.id) == nil
  end

  # Flipping a tour's privacy on Komoot is the supported way to publish a ride
  # here, and Komoot does not always bump changed_at for that edit alone.
  test "a tour made public on Komoot goes public here without a changed_at bump" do
    private = Map.merge(@enriched_tour, %{"status" => "private"})
    stub_komoot(tours: %{"tour_recorded" => [private], "tour_planned" => []})
    assert {:ok, %{imported: 1}} = KomootSync.sync()
    assert Rides.list_recorded_rides() == []

    # Same changed_at, privacy flipped.
    published = Map.merge(@enriched_tour, %{"status" => "public"})
    stub_komoot(tours: %{"tour_recorded" => [published], "tour_planned" => []})
    ...
    assert {:ok, %{updated: 0, skipped: 1}} = KomootSync.sync()
  end

  test "a tour made private on Komoot disappears from public queries" do
    stub_komoot(tours: %{"tour_recorded" => [@enriched_tour], "tour_planned" => []})
    assert {:ok, %{imported: 1}} = KomootSync.sync()
    assert [_ride] = Rides.list_recorded_rides()

    hidden = Map.merge(@enriched_tour, %{"status" => "private"})
    stub_komoot(tours: %{"tour_recorded" => [hidden], "tour_planned" => []})
    assert {:ok, %{updated: 1}} = KomootSync.sync()

    assert Rides.list_recorded_rides() == []
  end

  test "re-sync updates edited tours via changed_at and skips unchanged ones" do
    stub_komoot(tours: %{"tour_recorded" => [@enriched_tour], "tour_planned" => []})
    assert {:ok, %{imported: 1}} = KomootSync.sync()

    renamed =
      Map.merge(@enriched_tour, %{
        "name" => "Renamed loop",
        "status" => "private",
        "changed_at" => "2026-06-14T10:00:00.000Z"
      })

    stub_komoot(tours: %{"tour_recorded" => [renamed], "tour_planned" => []})
    assert {:ok, %{imported: 0, updated: 1, failed: 0}} = KomootSync.sync()
  ...
  test "backfills rides imported before the enrichment columns existed" do
    stub_komoot()
    assert {:ok, %{imported: 2}} = KomootSync.sync()

    enriched_existing =
      Map.merge(@recorded_tour, %{
        "changed_at" => "2026-06-13T10:00:00.000Z",
        "kcal_active" => 900
      })

    stub_komoot(
      tours: %{"tour_recorded" => [enriched_existing], "tour_planned" => [@planned_tour]}
    )

    assert {:ok, %{imported: 0, updated: 1, skipped: 1}} = KomootSync.sync()

    assert [ride] = Rides.list_recorded_rides()
    assert ride.kcal == 900
  end

  @tag :capture_log
  test "thumbnail download failure does not fail the import" do
    stub_komoot(
      tours: %{"tour_recorded" => [@enriched_tour], "tour_planned" => []},
      broken_images: true
    )
