defmodule WebWeb.RidesLiveTest do
  ...
  test "index shows the empty state when nothing is imported", %{conn: conn} do
    ...
    assert html =~ "Nothing public yet"
  end
  ...
  # Recorded activity leads the page; planned routes are a secondary section
  # underneath it rather than an equal column.
  test "index lists planned routes below recent activity", %{conn: conn} do
    points = [
    ...
    # Activity comes first in the document, planned routes after it.
    assert :binary.match(html, "Recent activity") < :binary.match(html, "Planned routes")
  end

  test "index omits the planned section entirely with nothing planned", %{conn: conn} do
    {:ok, _ride} = Rides.import_gpx(gpx_fixture(), %{"name" => "Lakes loop"})

    {:ok, _view, html} = live(conn, ~p"/fitness/rides")
    refute html =~ "Planned routes"
    assert html =~ "Recent activity"
  end
