...
// Mirrors @scored_fields in lib/web/fitness/biometric.ex — keep the two in step.
// The goals themselves arrive with the data (Biometric.goals/0), since they are
// the author's own numbers and live in the fitness vault, not in code.
const WATERFALL_METRICS = [
  { key: "sleep_hours", label: "Sleep", dir: "higher" },
  { key: "hrv_ms", label: "HRV", dir: "higher" },
  { key: "energy", label: "Energy", dir: "higher" },
  { key: "protein_grams", label: "Protein", dir: "higher" },
  { key: "soreness", label: "Soreness", dir: "lower" },
  { key: "calories_in", label: "Calories", dir: "target" },
  { key: "resting_hr", label: "RHR", dir: "lower" },
  { key: "fiber_grams", label: "Fiber", dir: "higher" },
  { key: "water_oz", label: "Water", dir: "higher" },
  { key: "active_calories", label: "Active cal", dir: "higher" },
]

const WEIGHTS = {
  sleep_hours: 3, hrv_ms: 3, energy: 2, protein_grams: 2, soreness: 2,
  calories_in: 1.5, resting_hr: 1.5, fiber_grams: 1, water_oz: 1,
  active_calories: 1,
}
...
function pct(v, goal, dir) {
  ...
  if (dir === "target") return Math.max(100 - Math.abs(v - goal) / goal * 200, 0)
  return Math.max(Math.min((2 * goal - v) / goal * 100, 100), 0)
}

export function renderWaterfall(canvas, entries, goals = {}) {
  ...
  const present = WATERFALL_METRICS.filter(m => latest[m.key] != null && goals[m.key] != null)
  if (present.length === 0) {
  ...
  const bars = present.map(m => {
    const score = pct(latest[m.key], goals[m.key], m.dir)
...
export const BiometricCharts = {
  ...
  render() {
    ...
    if (this.chartType === "line") {
      ...
      renderWaterfall(this.el, entries, goals || {})
    }
  }
}
