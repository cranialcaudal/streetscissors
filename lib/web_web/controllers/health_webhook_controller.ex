defmodule WebWeb.HealthWebhookController do
  ...
  @metric_map %{
    ...
    "dietary_fiber" => {:fiber_grams, :round},
    "dietary_energy" => {:calories_in, :energy_to_kcal},
  ...
  @flat_fields ~w(hrv_ms resting_hr active_calories vo2_max spo2_percent
                  ...
                  fiber_grams calories_in body_fat_percentage
  ...
  defp parse_metric(%{"name" => name, "data" => data_points} = metric, acc) do
    case Map.get(@metric_map, name) do
      nil ->
        acc

      {field, converter} ->
        converter = resolve_units(converter, metric["units"])
  ...
  # Health Auto Export reports dietary energy in whichever unit the phone is set
  # to. Only kJ needs converting; anything else is already kcal.
  defp resolve_units(:energy_to_kcal, units) when is_binary(units) do
    if String.downcase(units) in ["kj", "kilojoules"], do: :kj_to_kcal, else: :round
  end

  defp resolve_units(:energy_to_kcal, _), do: :round
  defp resolve_units(converter, _), do: converter

  defp convert(nil, _), do: nil
  defp convert(v, :round), do: round(v * 1.0)
  defp convert(v, :kj_to_kcal), do: round(v * 0.239006)
