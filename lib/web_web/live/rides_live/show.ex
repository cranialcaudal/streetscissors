defmodule WebWeb.RidesLive.Show do
  ...
  def mount(%{"id" => id}, _session, socket) do
    # Private Komoot tours 404 rather than leak
    ride = Rides.get_public_ride(id) || raise Ecto.NoResultsError, queryable: Web.Rides.Ride
  ...
  def render(assigns) do
    ...
    <div class="rides-container steel">
      <header class="rides-header ride-show-header">
        <.link navigate={~p"/fitness/rides"} class="ride-back">&larr; all GPX action</.link>
      ...
      <div class="ride-stats-grid">
        ...
        <%= if @ride.kind == "recorded" do %>
          <div class="ride-stat">
            <span class="ride-stat-value">
              {Format.duration(@ride.time_in_motion_s || @ride.duration_s)}
            </span>
        ...
        <div :if={@ride.kcal} class="ride-stat">
          <span class="ride-stat-value">{@ride.kcal}</span>
          <span class="ride-stat-label">kcal</span>
        </div>
      </div>
      ...
      <div :if={@ride.komoot_id} class="ride-komoot-toggle-wrap">
        <button
          class="ride-komoot-toggle-btn"
          phx-click={
            JS.show(to: "#komoot-embed-#{@ride.id}") |> JS.hide(to: "#komoot-toggle-#{@ride.id}")
          }
          id={"komoot-toggle-#{@ride.id}"}
        >
          Also view on Komoot &darr;
        </button>
        <iframe
