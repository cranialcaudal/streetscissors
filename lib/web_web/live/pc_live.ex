defmodule WebWeb.PcLive do
  ...
  def mount(_params, _session, socket) do
    ...
    welcome_message = """
    #{machine_name} — the short way round
    ...
    #{signature_line(DateTime.utc_now())}
    INDEXED: #{counts}
  ...
  # Who the terminal belongs to is configuration, not code: MACHINE_NAME titles
  # it, and AUTHOR_NAME signs the banner and answers `whoami`. Unset, it is an
  # anonymous machine.
  defp machine_name, do: Application.get_env(:web, :machine_name) || "The Machine"
  ...
  defp signature_line(now) do
    stamp = Calendar.strftime(now, "%a %d %b %Y %I:%M:%S %p %Z")

    case author_name() do
      nil -> stamp
      name -> String.pad_trailing(name, 44) <> stamp
    end
  end
  ...
  defp user_id do
    case author_name() do
      nil -> "guest"
      name -> name |> String.split() |> hd() |> String.downcase()
    end
  end
    ...
    <div class="pc-container" id="pc-terminal" phx-hook="AutoScroll">
      ...
      <div class="pc-header">
        ...
        <div style="text-align: right;">
          <span style="opacity: 0.8; display: block;">MS-DOS Prompt - "{@machine_name}"</span>
  ...
  defp handle_single_cmd("whoami", cwd, _socket), do: {user_id(), cwd}
